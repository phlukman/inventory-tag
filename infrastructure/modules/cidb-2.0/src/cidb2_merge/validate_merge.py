...
def validate_merge(
    merged_file, source_directory, pattern="*.csv", exclude_pattern="merged"
):
    """
    ...
    for filename, arns in arns_by_file.items():
        ...
        if missing_in_file:
            logger.error(
                f"File {filename} has {len(missing_in_file)} ARNs missing from merged file"
            )
        else:
            logger.info(
                f"✓ All {len(arns)} ARNs from {filename} are present in merged file"
            )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Validate that merged CSV contains all rows from source files"
    )
    parser.add_argument(
        "-m",
        "--merged",
...
if __name__ == "__main__":
    ...
    if is_valid:
        logger.info(
            "✅ VALIDATION SUCCESSFUL: All ARNs from source files are present in the merged file"
        )
        logger.info(
            f"   Merged file contains {merged_count} unique ARNs from {source_count} source ARNs"
        )
        exit(0)
    else:
        logger.error(
            "❌ VALIDATION FAILED: Some ARNs from source files are missing in the merged file"
        )
        logger.error(
            f"   {len(missing_arns)} ARNs missing out of {source_count} total source ARNs"
        )
        exit(1)
