...
def get_awsconfig_resource_config_by_arn(account,resource_type, arn):
    try:
        ...
        if not EXECUTION_ENV:
            pass
            # TODO: Implement role assumption chaining for local testing
        else:
...
def messages_to_csv(messages, awsconfig_service_name, to_file=False):
            ...
            with open('sqs_messages.csv', 'w', newline='') as csvfile:
                ...
                for message in messages:
                    try:
                        ...
                        #-----------------------------------------------
                        _, aws_config_conf = decode_aws_config_policy(aws_config_conf) if aws_config_conf else 'N/A'
                        # Write row to CSV
                        ...
                        row_data = {
                            ...
                            'AWSConfig': aws_config_conf
                        }
                        
                        if len(row_data['AWSConfig']) > 32767:
                            row_data['AWSConfig'] = row_data['AWSConfig'][:32767 - 3] + '...'  # Truncate if too long
                            logger.warning(f"Truncated AWSConfig for ARN {policy_arn}")
                        if len(row_data) < 4:
                            logger.warning(f"Row data incomplete for ARN {policy_arn}")
    ...
    try:
        for message in messages:
            try:
                # Extract fields from the message
                policy_arn =message.get('message',{}).get('data', {}).get('PolicyArn', 'N/A')
                if policy_arn == 'N/A':
                ...
                # Create CSV row
                aws_config_conf_undecoded = json.dumps(aws_config_conf, cls=DateTimeEncoder) if aws_config_conf else 'N/A'
                _, aws_config_conf_decoded = decode_aws_config_policy(json.dumps(aws_config_conf, cls=DateTimeEncoder)) if aws_config_conf else 'N/A'
                csv_rows.append({
                    'Type': policy_name if policy_name else 'N/A',
                    'Arn': policy_arn if policy_arn else 'N/A',
                    'Tags': json.dumps(policy_tags) if policy_tags else 'N/A',
                    'AWSConfig': aws_config_conf_decoded
                })
                # Error processing message: list indices must be integers or slices, not str Traceback
                # if len(csv_rows['AWSConfig']) > 32767:
                #     csv_rows['AWSConfig'] = csv_rows['AWSConfig'][:32767 - 3] + '...'  # Truncate if too long
                #     logger.warning(f"Truncated AWSConfig for ARN {policy_arn}")                
                # DEBUG
                #logger.info(f"csv row:{csv_rows[-1]}")
